...
for (i in 2:chain_length){
  ...
  for (j in 1:11){
    ...
    threshold <- cpp_ss_likelihood(C_obs, C_samples[, , i-1], 1 / prec_obs_sample[i-1,], Cpred_star, var_add, 
                                   c(100,9200,100,20,11000), c(4,4,4,4,4), 730, Gpred_star, G_e, ind = missing_data_ind - 1) - 
      cpp_ss_likelihood(C_obs, C_samples[, , i-1], 1 / prec_obs_sample[i-1,], Cpredi, var_add, 
                        c(100,9200,100,20,11000), c(4,4,4,4,4), 730, Gpred, G_e, ind = missing_data_ind - 1)
  ...
  for (j in 1:5){
    SSCobsi = sum((C_samples[missing_data_ind,j,i] - C_obs[missing_data_ind,j])^2)
    prec_obs_sample[i, j] = rgamma(1,length(missing_data_ind)/2, .5*SSCobsi + 1)
  }
}
## ADD GPP PRECISION
#})
