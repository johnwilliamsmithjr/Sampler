...
da <- darg(list(mle = TRUE, max = 10), randomLHS(1000,dim(Xinit)[2]))
gpi <- newGPsep(matrix(missing_data_ind/730), C_obs[missing_data_ind, 1], d = da$start, g = 1e-6, dK = TRUE)
mle <- mleGPsep(gpi, param = 'd', tmin = da$min, tmax = da$max, ab = da$ab)
p.Cf <- predGPsep(gpi, matrix(1:730)/730)
plot(p.Cf$mean, type = 'l')
points(C[,1], type = 'l', col = 'red')
deleteGPsep(gpi)

da <- darg(list(mle = TRUE, max = 10), randomLHS(1000,dim(Xinit)[2]))
gpi <- newGPsep(matrix(missing_data_ind/730), C_obs[missing_data_ind, 2], d = da$start, g = 1e-6, dK = TRUE)
mle <- mleGPsep(gpi, param = 'd', tmin = da$min, tmax = da$max, ab = da$ab)
p.Cw <- predGPsep(gpi, matrix(1:730)/730)
plot(p.Cw$mean, type = 'l')
points(C[,2], type = 'l', col = 'red')
deleteGPsep(gpi)

Cr.approx <- approxfun(missing_data_ind, C_obs[missing_data_ind,3])
plot(Cr.approx(1:730), type = 'l')
points(C[,3], col = 'red', type = 'l')

Csom.approx <- approxfun(missing_data_ind, C_obs[missing_data_ind,4])
plot(Csom.approx(1:730), type = 'l')
points(C[,4], col = 'red', type = 'l')

da <- darg(list(mle = TRUE, max = 10), randomLHS(1000,dim(Xinit)[2]))
gpi <- newGPsep(matrix(missing_data_ind/730), C_obs[missing_data_ind, 5], d = da$start, g = 1e-6, dK = TRUE)
mle <- mleGPsep(gpi, param = 'd', tmin = da$min, tmax = da$max, ab = da$ab)
p.Csom <- predGPsep(gpi, matrix(1:730)/730)
plot(p.Csom$mean, type = 'l')
points(C[,5], type = 'l', col = 'red')
deleteGPsep(gpi)
...
C_samples[, , 1] <- cbind(p.Cf$mean, p.Cw$mean, Cr.approx(1:730), Csom.approx(1:730), p.Csom$mean)
#sd[11] = .05
#sd[2] = .05
#sd[3] = .03
...
for (i in 2:chain_length){
  ...
  for (k in 1:5){
    ...
    if (i%%10 == 0 & i > start.adapt & mean(acceptance[(burn:i), k]) < .25){
      prec_sd[k] <- prec_sd[k] / 1.05
    } else if (i%%50 == 0 & i > start.adapt & mean(acceptance[(burn:i), k]) > .3){
      prec_sd[k] <- prec_sd[k] * 1.05
    }
  }
  ...
  #for (j in 1:5){
  #  SSCobsi = sum((C_samples[,j,i] - Cpredi[,j])^2)
  #  prec_add_sample[i, j] = rgamma(1,730/2 + 1, .5*SSCobsi + 1)
  #}
}
## ADD GPP PRECISION
#})
...
for (j in 1:5){
  ...
  hist(prec_add_sample[burn:chain_length, j], breaks = 50, main = paste0('True Value is ', prec_add[j]))
  #dev.off()
...
points(rowQuantiles(C_samples[,5,burn:chain_length], prob = .05), type = 'l', col = 'red', lty = 2)
points(rowQuantiles(C_samples[,5,burn:chain_length], prob = .95), type = 'l', col = 'red', lty = 2)
legend('topleft', legend = c('True Latent State', 'Estimated Latent State', 'Credible Bounds'), col = c('black', 'red', 'red'), lty = c(1,1,2))
dev.off()
